  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (data.error) alert(data.error);
            ...
            window.location.href = data.role === 'MOTORIST' ? '/user/dashboard' : '/provider/dashboard';
        }
    } catch (err) {
    ...
    <div style={{ display: 'flex', minHeight: '100vh', padding: '8rem 2rem', alignItems: 'center', justifyContent: 'center' }}>
      <div className="glass-panel animate-fade-up" style={{ width: '100%', maxWidth: '480px', padding: '4rem 3rem' }}>
        <header style={{ textAlign: 'center', marginBottom: '3rem' }}>
          ...
          <p className="text-muted">Enter your secure motorist or supplier key</p>
        </header>
        ...
        <footer style={{ marginTop: '3rem', textAlign: 'center', paddingTop: '2rem', borderTop: '1px solid hsl(var(--glass-border))' }}>
           <p className="text-muted" style={{ fontSize: '0.9rem' }}>
             New to Demand Roadside? <Link href="/register" style={{ color: 'hsl(var(--primary))', fontWeight: 700, textDecoration: 'none' }}>Join Network</Link>
           </p>
        </footer>
      </div>
    </div>
