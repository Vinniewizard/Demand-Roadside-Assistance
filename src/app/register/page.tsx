  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.error) alert(data.error);
        ...
        window.location.href = role === 'Drivers' ? '/user/dashboard' : '/provider/dashboard';
      }
    } catch (err) {
    ...
    <div style={{ display: 'flex', minHeight: '100vh', padding: '8rem 2rem 4rem', alignItems: 'center', justifyContent: 'center' }}>
      <div className="glass-panel animate-fade-up" style={{ width: '100%', maxWidth: '600px', padding: '4rem' }}>
        <header style={{ textAlign: 'center', marginBottom: '3rem' }}>
          ...
          <p className="text-muted">Register as a Drivers or a Kericho Service Supplier</p>
        </header>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '2.5rem' }}>
          <div onClick={() => setRole('Drivers')} className={`glass-panel`} style={{ padding: '1rem', textAlign: 'center', cursor: 'pointer', border: role === 'Drivers' ? '2px solid hsl(var(--primary))' : '1px solid transparent' }}>
            <User size={24} color={role === 'Drivers' ? 'hsl(var(--primary))' : 'inherit'} style={{ margin: '0 auto 0.5rem' }} />
            <p style={{ fontWeight: 800, fontSize: '0.8rem' }}>Drivers</p>
          </div>
