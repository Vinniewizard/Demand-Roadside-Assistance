        ...
        <div className="container-main" style={{ padding: '4rem 2rem', maxWidth: '1600px', margin: '0 auto' }}>
            <header className="header-mobile" style={{ marginBottom: '4rem', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-start', gap: '2rem' }}>
                <div style={{ flex: '1', minWidth: '300px' }}>
                    <h1 className="font-heading text-gradient" style={{ fontSize: 'clamp(2rem, 5vw, 3rem)', fontWeight: 900, lineHeight: 1.1 }}>Drivers Control</h1>
                    <p className="text-muted" style={{ fontSize: 'clamp(0.85rem, 2vw, 1rem)', marginTop: '0.5rem' }}>On Demand Roadside Assistance Platform in Kericho County</p>
            ...
            <div className="grid-responsive" style={{ alignItems: 'start' }}>
                ...
                <section className="glass-panel" style={{ padding: '2.5rem' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                        ...
                        <div className="animate-fade-up">
                            <label style={{ fontSize: '0.75rem', fontWeight: 900, color: 'hsl(var(--primary))', display: 'block', marginBottom: '0.8rem' }}>MISSION LOCATION</label>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                                <div style={{ padding: '1rem', background: isLocationConfirmed ? 'hsla(var(--success) / 0.1)' : 'hsla(var(--primary) / 0.05)', border: isLocationConfirmed ? '1px solid hsla(var(--success) / 0.3)' : '1px solid hsla(var(--primary) / 0.1)', borderRadius: '15px', color: isLocationConfirmed ? 'hsl(var(--success))' : 'inherit', fontSize: '0.75rem', fontWeight: 900, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.5rem', transition: 'all 0.3s ease' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                        <Navigation size={14} className={isLocationConfirmed ? '' : 'animate-pulse'} />
                                        {isLocationConfirmed ? 'LOCATION LOCKED' : 'GPS AUTO-DETECTING...'}
                                    </div>
                                    {isLocationConfirmed && <CheckCircle size={14} />}
                                </div>

                                {!isLocationConfirmed ? (
                                    <button onClick={() => { setIsLocationConfirmed(true); alert('Current GPS coordinates locked for this mission.'); }} className="btn-premium btn-outline" style={{ width: '100%', fontSize: '0.75rem', justifyContent: 'center' }}>
                                        <MapPin size={16} /> CONFIRM & SUBMIT MY CURRENT LOCATION
                                    </button>
                                ) : (
                                    <button onClick={() => setIsLocationConfirmed(false)} className="btn-premium btn-outline" style={{ width: '100%', fontSize: '0.75rem', justifyContent: 'center', opacity: 0.5 }}>
                                        CHANGE LOCATION
                                    </button>
                                )}
                            </div>
                        ...
                        <button
                            ...
                            style={{ width: '100%', padding: '1.5rem', fontSize: 'clamp(1rem, 2vw, 1.4rem)', justifyContent: 'center', opacity: (hasActiveRequest || !isLocationConfirmed) ? 0.3 : 1, marginTop: '1rem' }}>
                            {loading ? (
                                ...
                                <>{hasActiveRequest ? 'SYSTEM LOCKED: DISPATCH ACTIVE' : (isLocationConfirmed ? 'LAUNCH DISPATCH NOW' : 'VERIFY LOCATION FIRST')}</>
                            )}
                        </button>
                    </div>
                </section>
